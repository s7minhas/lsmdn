...
timeLevel = c('yearly', 'quarterly') # yearly or quarterly
vars = c(
  "my_tables.matlCoop2_netEvolve",
  "my_tables.matlCoop2_nonMil_netEvolve",
  "my_tables.matlCoop2_mil_netEvolve")
runMat = expand.grid(timeLevel=timeLevel, vars=vars, stringsAsFactors=FALSE)
runMat$varNames = gsub('_netEvolve','',gsub('my_tables.','',runMat$vars))
####

# cl = makeCluster(3)
# registerDoParallel(cl)
# foreach(ii = 1:nrow(runMat)) %dopar% {
ii=2

  ###Y should be of dimension nxnxT
  load( paste0(inPath, 'matlCoop_', runMat$timeLevel[ii], '.rda') )
  outFileName = paste0(outPath, 'LSMDN_', runMat$varNames[ii], '_', runMat$timeLevel[ii], '.rda')
  print(paste0('Writing to ', outFileName))
  Y = matlCoopArrList[[runMat$vars[ii]]]
  ###
