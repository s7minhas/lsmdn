...
if(printTrace){jpeg(file=paste0(graphicsPath, "Bin.jpg"))}
par(mar=c(4, 6, 2, 2) + 0.1)
plot(Bin[1:N],type="l",ylab=expression(beta[IN]),
     xlab="",cex.lab=2.5,cex.axis=1.5)
if(printTrace){dev.off()}
if(printTrace){jpeg(file=paste0(graphicsPath, "Bout.jpg"))}
par(mar=c(4, 6, 2, 2) + 0.1)
plot(Bout[1:N],type="l",ylab=expression(beta[OUT]),
     xlab="",cex.lab=2.5,cex.axis=1.5)
if(printTrace){dev.off()}
if(printTrace){jpeg(file=paste0(graphicsPath, "s2.jpg"))}
par(mar=c(4, 6, 2, 2) + 0.1)
plot(s2[1:N],type="l",ylab=expression(sigma^2),
     xlab="",cex.lab=2.5,cex.axis=1.5)
if(printTrace){dev.off()}
if(printTrace){jpeg(file=paste0(graphicsPath, "t2.jpg"))}
par(mar=c(4, 6, 2, 2) + 0.1)
...
jpeg(file=paste0(graphicsPath, "latentPositions.jpg"))
par(mar=c(2,2,4,2))
for(tt in 1:TT){
...
# for(i in 1:(n-1)){
#   for( j in (i+1):n){ 
#     kappaij[i,j] <- c.postzeroprob(X1[i,,],X2[i,,],X1[j,,],X2[j,,],s2[burnin:N],
#                                    lambda,p0)
#     kappaij[j,i] <- c.postzeroprob(X1[j,,],X2[j,,],X1[i,,],X2[i,,],s2[burnin:N],
#                                    lambda,p0)
#     count = count+1
